...
| **C6 — smaller ECCP sets with valid coverage.** | The full CCP verifier requires P2E to be strictly shorter in all 36/36 matched calibrator comparisons, including 9/9 AoN and 27/27 formula-faithful classical comparisons; every classical reduction must exceed the predeclared 10% materiality threshold. | `outputs/paper_headline_comparison.json` compares all 488 reported mean/SD scalars across 122 paper-table cells. All 376 unaffected scalars must pass. One exact CA finite-seed length-SD scalar (`0.201898618094345` reproduced versus `0.18` reported) is disclosed while its other three scalars pass. `outputs/ccp_calibrator_contract_audit.json` hash-binds and classifies 108 CCP scalars: the paper defines F1/F2/F3 as log/square-root/linear, but the released CCP driver fills those positions with square-root/log/power. The reversible log/square-root swap must independently replay all 72 available source-position scalars within the same fixed tolerances. No unexpected drift, incomplete/non-finite cell, failed efficiency rule, or failed P2E coverage sanity cell is allowed. |
