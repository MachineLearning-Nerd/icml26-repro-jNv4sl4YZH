...
def audit(candidate: Path, parent: Path) -> dict[str, object]:
    ...
    failures: list[str] = []
    if not (candidate / CAPSULE_PATH).is_file():
        failures.append(f"missing compact evidence capsule: {CAPSULE_PATH}")
    if CAPSULE_MARKER in parent_prompt:
        failures.append("historical negative control contains capsule marker")
    ...
    if CAPSULE_END_MARKER not in candidate_prompt:
        failures.append("candidate prompt truncates evidence capsule")
    missing_markers = [
        marker for marker in CLAIM_MARKERS if marker not in candidate_prompt
    ]
    if missing_markers:
        failures.append("candidate prompt omits claim markers")
    ...
    paths = [row["path"] for row in candidate_trace]
    if paths[:2] != ["pages/index.md", CAPSULE_PATH]:
        failures.append("candidate prompt ordering prefix is wrong")
    parent_markers_absent = not any(
        marker in parent_prompt for marker in CLAIM_MARKERS
    )
    if not parent_markers_absent:
        failures.append("historical prompt contains current exact-claim markers")
    return {
        ...
        "historical_negative_control": {
            ...
            "exact_claim_markers_absent": parent_markers_absent,
        },
        "candidate": {
            ...
            "all_six_claim_markers_visible": not missing_markers,
        },
